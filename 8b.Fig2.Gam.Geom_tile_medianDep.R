#####################################################
### Fig. 2: GAM+geom_tile: Median dep vs daylight ###
#####################################################
...
# panel c: GAM
################
ind_pred <- readRDS("./RDATA/4.GAM/Indiv-GAM_daylength_mediandep_slope_intercept_all.rds")
pop_pred <- readRDS("./RDATA/4.GAM/Pop-GAM_daylength_mediandep_slope_intercept_all.rds")
m <- readRDS("./RDATA/4.GAM/GAM_output_daylength_mediandep_slope_intercept_all.rds")
...
c = ggplot(pop_pred, aes(x = daylength, y = fit_pop)) +
  ...
  labs(y = "Daily median depth (m)", x = "Daylength (hours)", 
...
((a / b) | c) + plot_layout(widths  = c(1.5, 1),
                            heights = c(1,2))
ggsave(filename=paste0("./FIGURES/PAPER/Fig.2_geom-tile_GAM_mediandep.pdf"),
