...
#   # geom_histogram(alpha=.6,lwd=0.2) +
#   coord_flip() +
...
#             aes(x=values, y = 0.04, label=text), 
...
#            aes(x=values, fill=day_night, colour=day_night), 
#            colour="gray47") +
#   # geom_density(aes(y=after_stat(scaled)),alpha=.6,lwd=0.2) + 
#   geom_density(alpha=.6,lwd=0.2) + 
#   coord_flip() +
#   scale_x_continuous(trans = "reverse", limits = c(210, 0)) + 
#   # scale_y_continuous(breaks=c(0, 0.25, 0.5, 0.75), labels = percent) +
#   scale_fill_manual(values = c("navajowhite1","#595b54")) +
#   labs(y = "Density", x = "Daily mean depth (m)", 
#        title = "a) Dive depth", fill="") +
#   facet_wrap(~month, ncol=6, scales="free_y") +
#   theme_bw() +
#   geom_text(data=stat_meandep, 
#             aes(x=165, y = 0.5, label=p.adj.signif),
#             size=2, colour = "#595b54") +
#   geom_text(data=text_day_meandep, 
#             aes(x=values, y = 0.5, label=text), 
#             size=2, colour = "navajowhite3") + 
#   geom_text(data=text_night_meandep, 
#             aes(x=values, y = 0.5, label=text), 
#             size=2, colour = "#595b54") + 
...
(a / b) + plot_layout(heights = c(1,1.4),
                      width   = c(1,1))
ggsave(filename=paste0("./FIGURES/PAPER/Fig.2_density_DiffScale.pdf"),
       grid.arrange(a,b,ncol=1),
       width=7,height=5,units="in",dpi=400,family="ArialMT")

# ggsave(filename=paste0("./FIGURES/PAPER/Fig.2_density_SameScale.pdf"),
#        grid.arrange(a,b,ncol=1),
#        width=7,height=5,units="in",dpi=400,family="ArialMT")


nrow(dive[dive$maxdep>100,]) / nrow(dive) * 100 # 20%
nrow(dive[dive$maxdep>50,]) / nrow(dive) * 100  # 41%
mean(dive$maxdep)
...
month$meandep[month$month == "Dec"] / month$meandep[month$month == "Jul"] # 2.99


# test = dive %>% 
#   filter(id == "22849b" & date == "2014-07-22") %>%
#   select(id, date, dive, start_local, start, sunrise, sunset) 
# test
# unique(test$date)
# unique(test$sunrise)
# unique(test$sunset)
