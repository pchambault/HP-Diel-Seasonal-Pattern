...
## Daylength over time
```{r, echo=F}
daylight3 <- readRDS("./RDATA/4.daylight_HR.RDS") %>%
...
ggplot(daylight3, aes(x = date2, y = daylight)) +
...
```{r, echo=F}
cor_dat = daylight3 %>%
  select(-c(id,date,date2,month))
corr <- round(cor(cor_dat, use = "pairwise.complete.obs"), 1)
corr[is.na(corr)] <- 0
corr_p <- cor_pmat(cor_dat)
corr_p[is.na(corr_p)] <- 1

cor.test(cor_dat$mean_dep, cor_dat$daylight)
cor.test(cor_dat$median_dep, cor_dat$daylight)
cor.test(cor_dat$max_dep, cor_dat$daylight)
```
...
ggcorrplot(
  ...
  sig.level = 0.05)
```
Higher correlation between _meandep_ and _daylight_ (-0.8) compared to _mediandep_ and _daylight_ (-0.7) or  _maxdep_ (-0.4).


## Model summary
```{r, echo=F}
ind_pred <- readRDS("./RDATA/4.GAM/Indiv-GAM_HR_daylight_meandep_slope_intercept.rds")
pop_pred <- readRDS("./RDATA/4.GAM/Pop-GAM_HR_daylight_meandep_slope_intercept.rds")
m <- readRDS("./RDATA/4.GAM/GAM_HR_output_daylight_meandep_slope_intercept.rds")
summary(m)
```
