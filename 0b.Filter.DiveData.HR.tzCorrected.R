...
# identify dives > 5 m
# extract coordinates closest in time form location dataset
...
#########################################################################
# hp <- readRDS("./RDATA/0b.HP_dive_HR_Not_filtered.RDS")
offset    = 0
threshold = 5 # threshold depth below which an underwater phase should be considered a dive
...
loc <- readRDS("./RDATA/0a.locations_filtered_17HP_tzCorrected.rds") %>% 
  dplyr::select(id, posix, bathy, lon, lat, bathy_200)  %>%
  # filter(unique(id) == unique(dive$id))
  filter(id == "27262" | id == "22849b" | id == "22850b" | 
         id == "27262b" | id == "93100") %>%
  rename(date = posix) # use posix not posix_local (in local timezone)
summary(loc$bathy_200)
unique(loc$id)
unique(first_dep$id)
names(loc)
names(first_dep)
first_dep = first_dep %>% rename(date = posix)
...
first_dep = first_dep %>% 
  rename(posix = date)
  # mutate(posix_ch = as.character(posix))

# join coordinates from first_dep to dive dataset
#-------------------------------------------------
names(dive)
# dive = dive %>% mutate(posix_ch = as.character(posix))
names(first_dep)
dive2 = dive %>%
...
# assign coordinates and bathy to whole dive for each dive
#--------------------------------------------------------
dive2 = dive2 %>%
  group_by(id, dive) %>%
...
hist(ndive_summary$bathy_200, xlab = "Bathy (m)")
...
#---------------------------
ndive_summary %>% filter(dur == 0) %>% View()       # 16,520 dives!
