...
names(lr)
length(unique(lr$id))  # 15 porpoises! same ids for some, should be 120
table(lr$id)
table(lr$instr)
# Mk10 Splash 
# 7657   5871 
...
########################################
# distinguish day, night, dusk, dawn
########################################
library(suncalc)
dep # 1 row=1 depth class
  
#----------------
## dive depth ##
#----------------
sun  = getSunlightTimes(data = dep, tz="America/Nuuk", 
                        keep=c("sunrise","sunset","dusk",
                               "dawn","night","nightEnd")) 
head(sun)
dep = cbind(dep, sun[,c("sunrise","sunset","dusk",
                       "dawn","night","nightEnd")]) %>%
  as_tibble()
names(dep)
...
## dive duration ##
#------------------
sun  = getSunlightTimes(data = dur, tz="America/Nuuk", 
                        keep=c("sunrise","sunset","dusk",
                               "dawn","night","nightEnd")) 
head(sun)
dur = cbind(dur, sun[,c("sunrise","sunset","dusk",
                         "dawn","night","nightEnd")]) %>%
  as_tibble()
names(dur)
...
#------------------------------------
dep = dep %>% rename(posix_local = dateTime)
dur = dur %>% rename(posix_local = dateTime)
...
dep$hour = as.numeric(substr(dep$posix_local, 12, 13))
dur$hour = as.numeric(substr(dur$posix_local, 12, 13))
...
# save data
#########################
saveRDS(dep2, "./RDATA/1c.depth_LR_1row_1dive_19ids.RDS")
saveRDS(dur2, "./RDATA/1c.duration_LR_1row_1dive_19ids.RDS")
saveRDS(dep, "./RDATA/1c.depth_Binned_LR_19ids.RDS")
saveRDS(dur, "./RDATA/1c.duration_Binned_LR_19ids.RDS")
...
###########################################
# dep <- readRDS("./RDATA/1c.depth_Binned_LR_1row_1dive_19ids.RDS")
names(dep)
daylength = dep %>%
  filter(day_depart != 1) %>%  # remove first day because never complete (starts in the middle of day)
  group_by(id, date) %>%
  summarise(daylength = as.numeric(difftime(sunset, sunrise, units = "hour"))) %>%
  ungroup()
daylength
...
daylength3 = daylength2 %>%
  group_by(id, daylength, date) %>%
...
summary(daylength3)
daylength3
length(unique(daylength3$id)) # 19 ids


# save data
#--------------
saveRDS(daylength3, "./RDATA/1c.daylength_depth_LR_19ids.RDS")
...
saveRDS(dep_daily, "./RDATA/1c.dailyDepth_LR.RDS")
...
ggplot(dataPlot, 
  ...
  theme_tq() +
  labs(x = "", y = "Hour (Ameica/Nuuk-2)", fill = "Median depth (m)") +
...
#   View()
# 
# ggplot(data = daily, aes(x=meandep, 
#                              fill=day_night, colour=day_night)) +
...
#        title= "Daily mean depth (low resolution tags)", 
#        fill="") +
#   theme_tq() +
#   facet_grid(~month) +
