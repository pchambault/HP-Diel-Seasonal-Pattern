...
#----------------------
sunpos = sunAngle(as.POSIXct(bathy_df$date, tz="America/Nuuk"), 
...
c = ggplot(shore, aes(long, lat)) +
  ...
  annotate("text", y = 70, x=-40.2, 
           label = paste0(ndive_dec," dives"), size=2) +
  geom_hline(yintercept = 66, colour = "blue", 
             linewidth = 0.3, linetype=2) +
...
###############################
(a | b | c) 
ggsave(filename=paste0("./PAPER/4.PRSB/Fig.1.pdf"),
       width=180,height=90,units="mm",dpi=400,family="ArialMT") # full page width
