...
# Cor tests
#------------
cor.test(dat$mean_dep, dat$daylength)   # -0.57
cor.test(dat$max_dep, dat$daylength)    # -0.30

cor.test(hr$mean_dep, hr$daylength)     # -0.79
cor.test(hr$max_dep, hr$daylength)      # -0.40

cor.test(lr$mean_dep, lr$daylength)     # -0.48
cor.test(lr$max_dep, lr$daylength)      # -0.28
...
ggpairs(data, title="Pairwise correlations between variables", 
        ...
        upper = list(continuous = wrap("cor", linewidth = 2))) +
  theme_tq()
...
################################################
# Run GAM: daily mean of maxdepth vs daylength
################################################
